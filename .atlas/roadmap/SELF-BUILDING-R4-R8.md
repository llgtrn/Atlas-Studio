...
- R4.3.9 — rather than searching for a fifth narrow vector, this generation asked whether `stacker::maybe_grow`'s prior failure (documented in R4.3.6) ruled out ALL stack-size-based mitigation or only its specific lazy-growth mechanism. Confirmed by isolated empirical experiment (a throwaway harness spawning `syn::parse_file` at varying `stack_size`s, deleted after recording the finding): an UNCONDITIONALLY large stack allocated upfront via `std::thread::Builder::stack_size` -- a categorically different mechanism from `stacker`'s conditional, already-falsified lazy growth -- survives adversarial bracket nesting past depth 20,000 at 256MiB, roughly two orders of magnitude beyond every one of the four confirmed vectors' crash thresholds (300/2,000/3,000/2,000 on a default-sized stack), while a 2MiB stack reproduces the documented depth-50,000 crash baseline exactly. `RustSemanticExtractor::extract` now runs both `syn::parse_file` and its own subsequent AST walk (a second, independent recursive-descent traversal of the identical adversarial tree, previously unexamined as its own risk surface) inside a `std::thread::scope`-bounded worker with this stack size, layered BEHIND the existing `max_structural_recursion_risk` heuristic (which still rejects obviously-adversarial input cheaply, before ever paying the cost of spawning the worker) rather than replacing it. This does not claim stack overflow is now impossible -- a sufficiently large stack is still finite, and overflow in any thread still aborts the whole process, unchanged from today -- it specifically raises the bar for the exact residual risk R4.3.7/R4.3.8 already named: an undiscovered fifth/sixth/... vector must now reach roughly two orders of magnitude deeper before it can matter. Full pre-existing test suite (491 tests) confirmed unchanged and passing, including the existing `RESOURCE_LIMIT` regression test exercised through the new threaded code path with zero behavioral change for every other case.
...
Canonical main now has a useful R4.8 bootstrap: multi-level self.field READ/WRITE extraction (see below), compound-assignment read-modify-write hardening, conservative panic-like macro candidates, and conservative filesystem/process/network-effect candidates. This is not R4.8 semantic closure.

Multi-level self.field access: the single-level-only `self.field` model had a real, confirmed defect, not merely a coverage gap -- `self.a.b = 5;` used to fall back through `self_field_ident`'s depth-1-only check and get re-dispatched into the general `Expr::Field` READ arm, fabricating a spurious Read of `a` alone and never recording the real write at all. Confirmed directly (extracted a minimal fixture and inspected the raw observations) before fixing. `self_field_path` now resolves an arbitrarily deep chain of NAMED field accesses ultimately rooted at `self` (`self.a.b`, `self.a.b.c`, ...) to one compound state-access event against the `.`-joined path, never decomposed into one event per level -- an intermediate place (`self.a` inside `self.a.b`) is a pure place computation, never itself a read, regardless of whether the whole chain is read from or written to. A tuple-index member anywhere in the chain (`self.0`, `self.a.0`, `self.field.0`) still bails the whole chain -- and, closing a second instance of the identical fabrication bug found while writing the fix's own adversarial tests, the fallback for an unresolvable chain no longer re-enters field-chain interpretation on a sub-chain that is itself another `Expr::Field` (`self.field.0`'s fallback used to walk `self.field` and wrongly re-report a bare Read of `field`); it still walks into a genuinely different expression kind (a call, a method call, ...) that might hide an unrelated self-access, verified by a dedicated test proving `self.helper(self.value).z` still finds the real `self.value` read while correctly reporting nothing for `.z` itself.

Filesystem effects: after this session's own recensus concluded CFG/security/dependency-census had no further actionable defect (three consecutive clean generations from three distinct methods), the next census turned to `EffectCategory`'s own declared-but-unmaterialized variants -- `FilesystemRead`/`FilesystemWrite`/`NetworkSend`/`NetworkReceive`/`ProcessSpawn`/`FfiCall`/`Persist`/`EmitEvent`/`AuthCheck`/`Alloc`/`Free`/`ExternalIo` had existed in the enum since its own introduction but only `Panic` was ever emitted. A free/path function call whose callee spelling has an exact `fs` module-path qualifier (`std::fs::read`, `fs::write`, `tokio::fs::read_dir`, ...) or is exactly `File::open`/`File::create`/`File::create_new` now produces an `Inferred` `FilesystemRead`/`FilesystemWrite` candidate, following `persistence.rs`'s own proven textual-spelling-candidate discipline exactly (never `Observed`, since no type/name resolution proves the qualifier actually resolves to `std::fs`). The qualifier check is an EXACT path-segment match, never a substring match, guarding against the same collision risk `max_structural_recursion_risk`'s own `as`-keyword scan already had to solve (`prefs::`/`myfs::`/`overlayfs::` must never be mistaken for `fs::` merely because their spelling contains the letters "fs" -- verified by a dedicated test). Bare method calls (`file.read_to_string(..)`, `socket.write_all(..)`) are deliberately excluded this wave: a method call's spelling carries no qualifying module/type, making it far more ambiguous than a free/path call's own explicit qualifier -- an explicit, named scope boundary (documented in `effect.rs`'s own module doc comment), not a silent omission. Network/process/FFI/persist/event/auth/alloc/free/external-IO effect categories remain unmaterialized, left for future generations rather than attempted all at once.

Process-spawn effects: the very next generation extended the identical pattern to `ProcessSpawn` via `Command::new` (`std::process::Command::new`, `tokio::process::Command::new`, ...) -- a free/path constructor call as unambiguous as `File::open`/`File::create` were, deliberately NOT keyed on a bare `.spawn()` method call, which is at least as commonly an async-runtime task spawn or a `std::thread::spawn` as a real process one (an even sharper version of the method-call ambiguity already documented for filesystem). `Command::new` alone does not prove a process is ever actually spawned (the builder could be discarded without `.spawn()`/`.output()`/`.status()`), consistent with every other candidate here being `Inferred`, never `Observed`. This generation's own verification caught a genuine near-miss worth recording: the new test fixtures' literal source text initially tripped `rust_extractor_has_no_graph_dependency_and_never_executes_repository_code` (a real self-check scanning this extractor's own `mod.rs`/`spelling.rs`/`tests.rs` source for the literal spelling of the process-spawn constructor, to prove the extractor itself never shells out) -- the fixture strings quoting example code containing that exact spelling were textually indistinguishable, to that scan, from a real call in the extractor's own implementation. Fixed by building the fixture and its assertion message from separately-literal pieces at runtime, matching that scan's own pre-existing comment explaining exactly this technique -- a real instance of this session's own verification machinery doing its job, not a false alarm dismissed.

Network effects: the next generation extended the same pattern once more, to `NetworkSend`/`NetworkReceive` via `TcpStream::connect`/`UnixStream::connect` (client/initiating role -> Send) and `TcpListener::bind`/`UnixListener::bind` (server/accepting role -> Receive). None of these four constructors literally send or receive a byte at the call site named -- like `Command::new`, each is a connection SETUP site, not proof data ever crossed it -- but they are the closest-fitting categories this enum declares for "which side of the wire this call sets the function up to be", a bounded-precision tolerance `PersistenceKind`'s own multi-operation split already accepts. `UdpSocket::bind` was deliberately excluded: UDP is connectionless, and `bind` alone establishes no client/server role at all (the identical call is used equally before sending or before receiving), so classifying it either way would not be honest. Verified against all four qualifiers (Tcp and Unix variants of both connect and bind) producing the exact expected direction, plus a negative test covering `UdpSocket::bind`, a lookalike type name (`FakeTcpListener::bind`), and a bare method call (`sock.connect(..)`) all correctly producing nothing.
