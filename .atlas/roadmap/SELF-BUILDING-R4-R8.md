...
A third gap, found by direct adversarial testing after the syntax-coverage sweep this session already ran against every other R4.5-R4.11 walker: a `let PAT = EXPR else { diverge }` statement's diverge block is currently invisible to `lower_stmts`'s successor computation entirely (`stmt_expr` returns `None` for every `syn::Stmt::Local`, so the whole statement -- diverge block included -- is skipped). A function whose only conditional early exit is a let-else diverge block is reported identically to one with no conditional exit at all. Explicitly acknowledged, not silently missed: `adapter::semantic::rust::cfg::let_else_diverge_block_is_a_known_unrepresented_cfg_gap` asserts the exact current (incomplete) successor graph as a falsifiable regression baseline, so a future fix is a deliberate, visible test change, not a silent behavior drift. Properly closing it requires the same kind of branch-splitting logic `lower_stmts` already applies to `if`/`match`/loop constructs (a "success" edge to the join continuation, plus recursively-lowered successors for the diverge block's own statements) -- deferred rather than attempted this wave, given the risk of an under-validated change to CFG's own successor-computation core versus the low cost of an honestly documented gap.
