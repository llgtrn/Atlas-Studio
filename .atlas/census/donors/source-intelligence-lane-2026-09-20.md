...
- Typed source identity exists only partially today; current graph still carries bootstrap string IDs and path strings. Verified precisely against real code (2026-09-23, cross-referenced with `.atlas/genome/technology/rust-analyzer-tree-sitter-source-identity.md`): `core::graph::engineering_graph::build_repository_graph`'s `File` node sets `identity: file.path.clone()` (a raw path string) and only two attributes (`language`, `bytes`) -- no content fingerprint or digest, even though `runtime::census::extraction` already computes a `ContentFingerprint` (`stable_id("content", source_text)`, FNV-1a) per successfully-extracted file. `build_repository_graph`'s signature (`source: &SourceReport, docs: &DocsReport`) has no path to receive that fingerprint even if it were threaded through -- `SourceReport`'s own `FileFact` type carries only `path`/`language`/`bytes`, no fingerprint field. This is a real, bounded implementation task once actually needed (thread `ContentFingerprint` from `extraction_batches` into `build_system_graph`'s callers, add it as a `File` node attribute for files that were successfully extracted), deliberately not implemented in this pass: it requires the same identity-strategy decision flagged as open in the rust-analyzer genome record (does Atlas want content-aware identity fused with path identity, as rust-analyzer's `FileId`+revision does, or kept as two separate axes as today?) rather than a mechanical field addition.
