...
G149 (ADR 0065): scoped policy is executable. `atlas_core::seal` types every blocker code as a `BlockerKind`; an unknown code is refused, never skipped. A `SealPolicy` gives each kind a disposition: REQUIRED_CLEAR, ACCEPTED_BOUNDED_RESIDUAL (an enumerated set or a count bound, with a reason) or OUT_OF_SCOPE (with a reason). Permitted blockers stay listed in the verdict. `validate_policy` refuses a policy that permits REVISION_DIRTY, DEPENDENCY_UNRESOLVED, PROVENANCE_INCOMPLETE, NORMALIZATION_CONFLICTS, REPLAY_NOT_CONVERGED or COVERAGE_CONFLICT, or that accepts UNKNOWN or UNSUPPORTED coverage of a HARD dimension. The policy is pinned by its digest identity. The repository declares its policy at `.atlas/declared/seal-policy.json`; `atlas-systemizer seal evaluate` applies it to a certificate. Under that policy the Atlas self-scope certificate is NOT_ELIGIBLE, and the verdict names every refused blocker.

G161 (ADR 0076): the seal gate (M8) decides a seal over a verified container: the certificate verdict recomputed under the policy and the certificate at least CLOSED, an ADMISSIBLE verification report and the policy's integrity verdict for the container's census digest and clean revision, and a SELECTED design; `atlas-systemizer seal gate` exits SEAL_NOT_ELIGIBLE with every typed reason, and `atlas seal` writes the SEALED container (M9) only from an ELIGIBLE decision. The self scope remains NOT_ELIGIBLE: a RECONCILED certificate with refused blockers, and no SELECTED design.
