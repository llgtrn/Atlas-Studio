...
- `section_flags` = 0: every section is required. No section flag marks a section skippable, so the reader refuses a flagged section and an unknown section type rather than skipping it. A new section type therefore needs a new `format_minor` that older readers refuse. WebAssembly's skippable custom sections have no counterpart until a cross-version reader exists (G97);
