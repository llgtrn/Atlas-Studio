...
1. No path under `.atlas/temporary/donors/**` may be named exactly `.claude`, `.codex`, or `.cursor` (as a directory), or exactly `CLAUDE.md`, `AGENTS.md`, or `.mcp.json` (as a file), at any depth. Donor content that arrives in one of these shapes is quarantined (renamed, contents fully preserved, never deleted or truncated) immediately upon staging, before census begins:
   ...
   - a `CLAUDE.md`/`AGENTS.md`/`.mcp.json` file gets the suffix `.donor-untrusted` appended (e.g. `CLAUDE.md` -> `CLAUDE.md.donor-untrusted`).
...
As of this contract's introduction, the scan is clean: 54 paths across 10 donors (`arrow`, `borg`, `buck2`, `duumbi`, `fastcdc-rs`, `iris`, `openrewrite`, `security/keycloak`, `wasmtime`, plus scattered root-level `CLAUDE.md`/`AGENTS.md`/`.mcp.json` files in several others) were quarantined under this contract's introduction.
...
- This contract does not attempt to enumerate every possible agent-tooling convention's file-naming scheme beyond what has actually been observed to matter (`.claude`, `.codex`, `.cursor`, `CLAUDE.md`, `AGENTS.md`, `.mcp.json`). If a new class of risk is discovered, extend the verification script's pattern list and this contract's invariant list together, and re-run the scan across all existing donors.
