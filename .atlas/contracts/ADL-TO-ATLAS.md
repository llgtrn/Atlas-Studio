...
- **Effect envelopes (G137, ADR 0055).** `forall f: function in <Entity> observed effect within <CATEGORY>, <CATEGORY>` declares an entity's effect envelope. `within none` declares an entity with no effect. Categories are upper-case names, sorted and de-duplicated at parse time; an empty list, a lower-case name or `none` mixed with categories is `ATLAS-E052`.
  - **What it claims.** Every definite (`OBSERVED` or `DERIVED`) effect site of the entity's functions lies inside the envelope. It is the declared/observed effect reconciliation, the counterpart of `depends_on` against the dependency census, and it claims nothing about guessed (`INFERRED`) sites or files where `EFFECT` is partial.
  - **How it is decided.** It is always decided by systemize (`UNKNOWN` only when the entity is not in the composed census). A definite site outside the envelope is `VIOLATED` (`ATLAS-E064`), with every such site named. Otherwise it is `SATISFIED`, and the basis names the `INFERRED` sites and unobserved files that the claim does not cover.
  - **How it relates to `forbid effect`.** `forbid effect` remains the universal claim that no such effect happens anywhere, and it stays `UNKNOWN` while `EFFECT` is partial.
  - **Derivation.** `adl derive` lowers the envelope of every composed entity that has functions and no authored envelope into `census.adl`, as `invariant <Entity>EffectEnvelope`, listing only categories and never counts. The model for the derivation is composed from the authored ADL plus the derived membership, so regeneration is a fixed point.
  - **What a new category triggers.** A category a later census observes outside the committed envelope is `VIOLATED`, which blocks admission. It also drifts `census.adl`, which fails `adl derive --check` and the `census_adl_is_current` test.
