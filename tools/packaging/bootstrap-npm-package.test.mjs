...
test("parseArgs recognizes publish and skip-build flags", () => {
  assert.deepEqual(parseArgs(["@chronica/adapter-acpx-local", "--publish", "--skip-build"]), {
    help: false,
    selector: "@chronica/adapter-acpx-local",
    publish: true,
    skipBuild: true,
    otp: null,
  });
...
test("resolveTargetPackage matches by package name or dir", () => {
  const packages = [
    { dir: "packages/a", name: "@chronica/a", pkg: {} },
    { dir: "packages/b", name: "@chronica/b", pkg: {} },
  ];

  assert.equal(resolveTargetPackage("@chronica/a", packages).dir, "packages/a");
  assert.equal(resolveTargetPackage("./packages/b", packages).name, "@chronica/b");
});

test("resolveTargetPackage includes the workspace diff plugin bootstrap package", () => {
  const pkg = resolveTargetPackage("@chronica/plugin-workspace-diff");
