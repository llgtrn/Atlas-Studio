...
import { buildReleasePackagePlan } from "../release/release-package-map.mjs";
...
function usage() {
  process.stderr.write(
    [
      "Usage:",
      "  node tools/packaging/bootstrap-npm-package.mjs <package-name-or-dir> [--publish --otp <code>] [--skip-build]",
      "",
      "Examples:",
      "  node tools/packaging/bootstrap-npm-package.mjs @chronica/adapter-acpx-local",
      "  node tools/packaging/bootstrap-npm-package.mjs packages/adapters/acpx-local --publish",
      "",
    ].join("\n"),
  );
}
  ...
  if (/\bE401\b|401 Unauthorized/i.test(output)) {
    throw new Error(
      [
        ...
        "Run `npm logout --registry=https://registry.npmjs.org/` and then `npm login` or `npm adduser` on this maintainer machine with an npm account that can publish to the @chronica scope, then rerun with --publish.",
...
function printNextSteps(pkg) {
  process.stdout.write(
    [
      ...
      "3. Add repository llgtrn/Chronica",
  ...
  if (!publish) {
    process.stdout.write(
      [
        ...
        `node tools/packaging/bootstrap-npm-package.mjs ${pkg.name} --publish --otp <code>`,
        "",
      ].join("\n"),
    );
    return;
