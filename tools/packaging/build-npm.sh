...
#
# Usage:
#   ./tools/packaging/build-npm.sh               # full build
#   ./tools/packaging/build-npm.sh --skip-checks  # skip forbidden-token check (CI without token list)
...
if [ "$skip_checks" = false ]; then
  ...
  node "$REPO_ROOT/tools/release/check-forbidden-tokens.mjs"
else
...
node "$REPO_ROOT/tools/packaging/generate-npm-package-json.mjs"
