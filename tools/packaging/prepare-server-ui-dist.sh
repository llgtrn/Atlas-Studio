...
# This keeps @chronica/server publish artifacts self-contained for static UI serving.
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @chronica/ui dist output"
else
  echo "  -> Building @chronica/ui..."
  pnpm --dir "$REPO_ROOT" --filter @chronica/ui build
fi
