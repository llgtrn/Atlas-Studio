...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@chronica/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[paperclip] Command failed with code ${status.code}: pnpm --filter @chronica/db exec tsx src/migration-status.ts --json\n`,
    );
...
async function buildPluginSdk() {
  ...
  const result = await runPnpm(
    ["--filter", "@chronica/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@chronica/server", serverScript, ...forwardedArgs],
