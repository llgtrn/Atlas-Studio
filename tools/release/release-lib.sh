...
wait_for_release_registry_state() {
  ...
  while [ "$attempt" -le "$attempts" ]; do
    if output="$(node "$REPO_ROOT/tools/release/verify-release-registry-state.mjs" "$@" 2>&1)"; then
      [ -n "$output" ] && printf '%s\n' "$output"
      return 0
    fi
    status=$?
...
list_public_package_info() {
  node "$REPO_ROOT/tools/release/release-package-map.mjs" list
}

set_public_package_version() {
  node "$REPO_ROOT/tools/release/release-package-map.mjs" set-version "$1"
}
