...
test("manifest changes without base state validate all release-enabled packages", () => {
  const releasePackages = [
    { dir: "packages/a", name: "@chronica/a", publishFromCi: true },
    { dir: "packages/b", name: "@chronica/b", publishFromCi: true },
    { dir: "packages/c", name: "@chronica/c", publishFromCi: false },
  ];

  const changedPackages = collectReleasePackagesForChangedPaths(
    ["tools/release/release-package-manifest.json"],
    releasePackages,
  );

  assert.deepEqual(
    changedPackages.map((pkg) => pkg.name),
    ["@chronica/a", "@chronica/b"],
  );
});
...
test("manifest changes only validate newly release-enabled packages relative to base state", () => {
  ...
  const baseReleaseState = {
    source: "manifest",
    byDir: new Map([["packages/a", { name: "@chronica/a", publishFromCi: true }]]),
  };
...
test("base release state falls back to public packages when manifest is absent", () => {
  ...
  const baseReleaseState = getBaseReleaseState("base-sha", releasePackages, (_revision, filePath) => {
    if (filePath === "tools/release/release-package-manifest.json") {
      return null;
    }

    if (filePath === "packages/a/package.json") {
      return JSON.stringify({ name: "@chronica/a", private: false });
    }

    if (filePath === "packages/b/package.json") {
      return JSON.stringify({ name: "@chronica/b", private: true });
    }
  ...
  assert.deepEqual([...baseReleaseState.byDir.entries()], [
    ["packages/a", { name: "@chronica/a", publishFromCi: true }],
  ]);
});
