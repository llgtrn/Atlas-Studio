...
function getBaseReleaseState(
  ...
  const manifestText = readFileAtRevision(revision, "tools/release/release-package-manifest.json");

  if (manifestText) {
    const manifestEntries = JSON.parse(manifestText);

    if (!Array.isArray(manifestEntries)) {
      throw new Error(`expected tools/release/release-package-manifest.json at ${revision} to contain an array`);
    }
...
function collectReleasePackagesForChangedPaths(
  ...
  const manifestFileChanged = normalizedChangedPaths.includes("tools/release/release-package-manifest.json");
...
if (process.argv[1] && normalizePath(process.argv[1]).endsWith("tools/release/check-release-package-bootstrap.mjs")) {
