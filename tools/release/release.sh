...
. "$REPO_ROOT/tools/release/release-lib.sh"
...
usage() {
  cat <<'EOF'
Usage:
  ./tools/release/release.sh <canary|stable> [--date YYYY-MM-DD] [--dry-run] [--skip-verify] [--print-version]

Examples:
  ./tools/release/release.sh canary
  ./tools/release/release.sh canary --date 2026-03-17 --dry-run
  ./tools/release/release.sh stable
  ./tools/release/release.sh stable --date 2026-03-17 --dry-run
  ./tools/release/release.sh stable --date 2026-03-18 --print-version
...
cd "$REPO_ROOT"
pnpm build
node "$REPO_ROOT/tools/packaging/build-standalone-public-packages.mjs"
bash "$REPO_ROOT/tools/packaging/prepare-server-ui-dist.sh"
for pkg_dir in server packages/adapters/claude-local packages/adapters/codex-local; do
  rm -rf "$REPO_ROOT/$pkg_dir/skills"
...
"$REPO_ROOT/tools/packaging/build-npm.sh" --skip-checks --skip-typecheck
...
if [ "$dry_run" = true ]; then
  ...
  if [ "$channel" = "canary" ]; then
    ...
    release_info "  ./tools/release/create-github-release.sh $TARGET_STABLE_VERSION"
  fi
fi
