...
test("collectInternalDependencyProblems flags missing internal versions", () => {
  const manifest = {
    dependencies: {
      "@chronica/plugin-sdk": "2026.425.0-canary.5",
      e2b: "^2.19.0",
    },
  };
  ...
  assert.deepEqual(
    ...
    ["dependencies requires @chronica/plugin-sdk@2026.425.0-canary.5, but npm does not expose that version"],
  );
});
...
test("collectInternalDependencyProblems accepts version-specific manifests when the root document is stale", () => {
  ...
  const packageManifestsByKey = new Map([
    [
      createManifestLookupKey("@chronica/plugin-sdk", "2026.425.0-canary.5"),
      { name: "@chronica/plugin-sdk", version: "2026.425.0-canary.5" },
    ],
  ]);
...
test("collectInternalDependencyProblems ignores peer dependency range specifiers", () => {
  const manifest = {
    peerDependencies: {
      "@chronica/server": "^2026.430.0-canary.0",
    },
  };
...
test("collectInternalDependencyProblems reports unfetched transitive dependency metadata neutrally", () => {
  const manifest = {
    optionalDependencies: {
      "@chronica/browser": "2026.430.0-canary.0",
    },
  };
  ...
  assert.deepEqual(
    collectInternalDependencyProblems(manifest, new Map()),
    [
      "optionalDependencies requires @chronica/browser@2026.430.0-canary.0, but npm publication metadata was not fetched for that dependency",
    ],
  );
});
...
test("verifyPackageRegistryState tolerates a stale root versions map when dist-tags and direct manifests are correct", () => {
  ...
  const packageManifestsByKey = new Map([
    [
      createManifestLookupKey("@chronica/ui", "2026.430.0-canary.0"),
      {
        name: "@chronica/ui",
        version: "2026.430.0-canary.0",
        dependencies: {
          "@chronica/shared": "2026.430.0-canary.0",
        },
      },
    ],
    [
      createManifestLookupKey("@chronica/shared", "2026.430.0-canary.0"),
      {
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "@chronica/ui",
      packageDoc: packageDocsByName.get("@chronica/ui"),
      packageDocsByName,
...
test("verifyPackageRegistryState fails when canary latest is left in place by default", () => {
    ...
    [
      "@chronica/plugin-e2b",
      {
        ...
        versions: {
          ...
          "2026.427.0-canary.3": {
            dependencies: {
              "@chronica/plugin-sdk": "2026.427.0-canary.3",
            },
          },
        },
      },
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "@chronica/plugin-e2b",
      packageDoc: packageDocsByName.get("@chronica/plugin-e2b"),
      packageDocsByName,
      channel: "canary",
      distTag: "canary",
    ...
    [
      "@chronica/plugin-e2b: latest dist-tag still resolves to canary 2026.425.0-canary.5; if that state is intentional, rerun the verification script directly with --allow-canary-latest",
      "@chronica/plugin-e2b@2026.425.0-canary.5 via latest: dependencies requires @chronica/plugin-sdk@2026.425.0-canary.5, but npm does not expose that version",
    ],
  );
});
    ...
    [
      "paperclipai",
      {
        ...
        versions: {
          "2026.427.0-canary.3": {
            dependencies: {
              "@chronica/server": "2026.427.0-canary.3",
            },
          },
        },
      },
...
test("verifyPackageRegistryState still fails when the dist-tag is stale", () => {
  ...
  assert.deepEqual(
    ...
    ["@chronica/ui: dist-tag canary resolves to 2026.429.0-canary.2, expected 2026.430.0-canary.0"],
  );
});
...
test("verifyPackageRegistryState ignores internal peer dependency ranges", () => {
  ...
  assert.deepEqual(
    verifyPackageRegistryState({
      packageName: "@chronica/plugin-sdk",
      packageDoc: packageDocsByName.get("@chronica/plugin-sdk"),
      packageDocsByName,
      channel: "canary",
      distTag: "canary",
      targetVersion: "2026.430.0-canary.0",
...
test("fetchRegistryJson times out hung requests", async () => {
  ...
  try {
    await assert.rejects(
      fetchRegistryJson(new URL("https://registry.npmjs.org/@chronica%2Fui"), { timeoutMs: 1 }),
      /timed out/,
    );
  } finally {
