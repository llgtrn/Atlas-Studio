...
const manifestPath = join(repoRoot, "tools", "release", "release-package-manifest.json");
const roots = ["apps/ui/packages", "apps/ui", "server", "cli"];
...
function replaceWorkspaceDeps(deps, version) {
  ...
  for (const [name, value] of Object.entries(next)) {
    if (!name.startsWith("@chronica/")) continue;
...
function usage() {
  process.stderr.write(
    [
      "Usage:",
      "  node tools/release/release-package-map.mjs list",
      "  node tools/release/release-package-map.mjs check",
      "  node tools/release/release-package-map.mjs set-version <version>",
      "",
    ].join("\n"),
  );
}
