...
function main() {
  ...
  // The root Dockerfile is the Rust kernel image (chronica-api). This check guards the
  // pnpm dependency-caching stage of a Node image; when the Dockerfile does not use pnpm
  // at all there is no deps stage to keep in sync with the pnpm workspace manifests.
  if (!/\bpnpm\b/.test(dockerfileText)) {
    console.log("PASS (root Dockerfile does not use pnpm — deps-stage sync check not applicable)");
    return;
  }

  const depsStage = extractDepsStage(dockerfileText);
  if (!depsStage.trim()) {
