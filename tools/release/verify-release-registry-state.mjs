...
function usage() {
  process.stderr.write(
    [
      "Usage:",
      "  node tools/release/verify-release-registry-state.mjs --channel <canary|stable> --dist-tag <tag> --target-version <version> --package <name> [--package <name> ...] [--allow-canary-latest]",
      "",
    ].join("\n"),
  );
}
...
function collectInternalDependencyProblemEntries(
  ...
  for (const [sectionName, deps] of sections) {
    for (const [dependencyName, dependencyVersion] of Object.entries(deps)) {
      if (!dependencyName.startsWith("@chronica/")) {
        continue;
      }
